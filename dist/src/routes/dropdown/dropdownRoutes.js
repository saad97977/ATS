 ...
 *   ?take=<number>    – result cap (default 50, max 200)
 *
 * Available endpoints:
 ...
 * @query   search (partial org name), take (default 50, max 200)
 * @access  HCM_USER
 * @example GET /api/dropdowns/organizations?search=acme&take=50
 */
router.get('/organizations', authMiddleware_1.authenticateToken, (0, authMiddleware_1.authorizeRole)('HCM_USER'), dropdownController_1.dropdownController.getOrganizations);
/**
 * @route   GET /api/dropdowns/jobs
 * @desc    Get jobs for dropdown/autocomplete
 * @query   search (job title or location), organization_id, status, take
 * @access  HCM_USER
 * @example GET /api/dropdowns/jobs?search=developer&organization_id=org123&status=OPEN&take=50
 */
router.get('/jobs', authMiddleware_1.authenticateToken, (0, authMiddleware_1.authorizeRole)('HCM_USER'), dropdownController_1.dropdownController.getJobs);
/**
 * @route   GET /api/dropdowns/organization-users
 * @desc    Get organization users for dropdown/autocomplete
 * @query   search (name or email), organization_id, take
 * @access  HCM_USER
 * @example GET /api/dropdowns/organization-users?search=john&organization_id=org123&take=50
 */
router.get('/organization-users', authMiddleware_1.authenticateToken, (0, authMiddleware_1.authorizeRole)('HCM_USER'), dropdownController_1.dropdownController.getOrganizationUsers);
/**
 * @route   GET /api/dropdowns/document-categories
 * @desc    Get document categories (titles) for dropdown/autocomplete
 * @query   search (category name), organization_id, take
 * @access  HCM_USER
 * @example GET /api/dropdowns/document-categories?search=contract&organization_id=org123&take=50
 */
router.get('/document-categories', authMiddleware_1.authenticateToken, (0, authMiddleware_1.authorizeRole)('HCM_USER'), dropdownController_1.dropdownController.getDocumentCategories);
