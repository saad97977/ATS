...
const submitApplicationSchema = zod_1.z.object({
    ...
    communication_preference: zod_1.z.string().optional(), // "Email, Text"
    ...
    comp_code_last: zod_1.z.string().optional(),
    ...
    education: zod_1.z.preprocess((val) => {
        if (typeof val === 'string') {
            try {
                return JSON.parse(val);
            }
...
async function findExistingApplicant(email) {
    return prisma_config_1.default.applicant.findFirst({
        where: { contact: { email } },
        include: { contact: true, demographic: true, social_profiles: true },
    });
}
...
const submitApplication = async (req, res) => {
    try {
        ...
        const jobId = req.params.jobId || undefined;
        const file = req.file;
        // ── Validate ─────────────────────────────────────────────────────────────
        ...
        if (!data.job_id) {
            ...
            const applicant = await prisma_config_1.default.$transaction(async (tx) => upsertApplicant(tx, data, existingApplicant), { maxWait: 8000, timeout: 20000 });
            // Education upsert (applicant-level, no application scope needed)
            if (data.education && data.education.length > 0) {
                await prisma_config_1.default.applicantEducation.createMany({
        ...
        const [job, existingApplicant] = await Promise.all([
            prisma_config_1.default.job.findFirst({
                where: {
                    job_id: data.job_id,
                    status: 'OPEN',
        ...
        if (existingApplicant) {
            const duplicate = await prisma_config_1.default.application.findFirst({
...
const uploadApplicationResume = async (req, res) => {
    try {
        ...
        if (!file) {
            return (0, response_1.sendError)(res, 'Resume file is required', 400);
        }
        if (!email) {
        ...
        if (!allowedMimeTypes.includes(file.mimetype)) {
            return (0, response_1.sendError)(res, 'Invalid file type. Only PDF, DOC, and DOCX files are allowed', 400);
        }
        // Validate file size (max 5MB)
        const maxSize = 5 * 1024 * 1024; // 5MB
        if (file.size > maxSize) {
            return (0, response_1.sendError)(res, 'File size too large. Maximum size is 5MB', 400);
        }
        ...
        if (application.applicant.contact?.email !== email) {
            return (0, response_1.sendError)(res, 'Email does not match the application', 403);
        }
        // Check if resume already exists for this application
        const existingResume = await prisma_config_1.default.applicantDocument.findFirst({
            where: {
        ...
        const containerClient = await getContainerClient();
        const blobName = generateBlobName(application.applicant_id, applicationId, file.originalname);
        const blockBlobClient = containerClient.getBlockBlobClient(blobName);
        await blockBlobClient.upload(file.buffer, file.buffer.length, {
            blobHTTPHeaders: { blobContentType: file.mimetype },
        });
        ...
        if (existingResume) {
            ...
            await prisma_config_1.default.applicantDocument.update({
        ...
        return (0, response_1.sendError)(res, 'Failed to upload resume', 500);
    }
};
