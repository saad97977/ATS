...
exports.listW2Employees = listW2Employees;
exports.generateW2PDF = generateW2PDF;
exports.printW2Batch = printW2Batch;
exports.list1099Contractors = list1099Contractors;
exports.generate1099PDF = generate1099PDF;
exports.notifyTaxDocumentAvailable = notifyTaxDocumentAvailable;
exports.recordTaxDocumentConsent = recordTaxDocumentConsent;
exports.payrollRegister = payrollRegister;
exports.payrollSummary = payrollSummary;
exports.earningsReport = earningsReport;
exports.deductionReport = deductionReport;
exports.taxLiabilityReport = taxLiabilityReport;
exports.achRegister = achRegister;
exports.invoiceRegister = invoiceRegister;
exports.revenueByCustomer = revenueByCustomer;
exports.revenueByEmployee = revenueByEmployee;
exports.revenueByBranch = revenueByBranch;
exports.outstandingInvoices = outstandingInvoices;
exports.customerBillingSummary = customerBillingSummary;
exports.acaEligibilityReport = acaEligibilityReport;
exports.workersCompensationReport = workersCompensationReport;
exports.paidSickLeaveReport = paidSickLeaveReport;
exports.employeeHoursReport = employeeHoursReport;
exports.w2EmployeesReport = w2EmployeesReport;
exports.contractors1099Report = contractors1099Report;
exports.newHireReport = newHireReport;
exports.oshaReport = oshaReport;
exports.federalEEOReport = federalEEOReport;
exports.accruedHoursReport = accruedHoursReport;
exports.activeAssignmentReport = activeAssignmentReport;
exports.deductionSubmittalReport = deductionSubmittalReport;
exports.employeeDeductionContributionReport = employeeDeductionContributionReport;
exports.getReportCatalog = getReportCatalog;
const prisma_config_1 = __importDefault(require("../../prisma.config"));
const pdfkit_1 = __importDefault(require("pdfkit")); // npm install pdfkit --save
const response_1 = require("../../utils/response");
// ─────────────────────────────────────────────────────────────────────────
// GENERIC EXPORT HELPERS (CSV / PDF / JSON)
// ─────────────────────────────────────────────────────────────────────────
function getFormat(req) {
    const f = (req.query.format || "csv").toLowerCase();
    return (["csv", "pdf", "json"].includes(f) ? f : "csv");
}
...
function sendPDFTable(res, filename, title, rows, columns, meta) {
    const landscape = columns.length > 6;
    const doc = new pdfkit_1.default({ margin: 30, size: "A4", layout: landscape ? "landscape" : "portrait" });
...
function respondWithReport(req, res, opts) {
    const format = getFormat(req);
    if (format === "json")
        return (0, response_1.sendSuccess)(res, { count: opts.rows.length, rows: opts.rows });
    if (format === "pdf")
        return sendPDFTable(res, opts.filename, opts.title, opts.rows, opts.columns, opts.meta);
...
function maskSSN(encrypted) {
    ...
    try {
        ...
        if (typeof enc.decrypt === "function") {
            const plain = enc.decrypt(encrypted);
 ...
 */
async function listW2Employees(req, res) {
    try {
        ...
        const checks = await prisma_config_1.default.payrollCheck.findMany({
            where: {
        ...
        const totalsByApplicant = new Map();
        for (const c of checks) {
            const key = c.applicant_id;
            const acc = totalsByApplicant.get(key) ??
        ...
        const applicants = applicantIds.length
            ? await prisma_config_1.default.applicant.findMany({
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "listW2Employees failed", 500);
    }
}
/**
 ...
 */
async function generateW2PDF(req, res) {
    try {
        ...
        doc
            ...
            .text("System-generated summary for electronic delivery. Official Copy A/B/C/1/2 issuance must follow current IRS formatting requirements.", { align: "center" });
        doc.moveDown(1.2);
        ...
        field("Address", [applicant.contact?.address, applicant.contact?.city, applicant.contact?.state, applicant.contact?.zip]
            .filter(Boolean)
            .join(", "));
        doc.moveDown(0.3);
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "generateW2PDF failed", 500);
    }
}
/**
 ...
 */
async function printW2Batch(req, res) {
    try {
        const taxYear = Number(req.query.taxYear ?? new Date().getFullYear());
        const idFilter = parseCSVParam(req.query.applicantIds);
        const yearStart = new Date(`${taxYear}-01-01T00:00:00.000Z`);
        const yearEnd = new Date(`${taxYear}-12-31T23:59:59.999Z`);
        const checks = await prisma_config_1.default.payrollCheck.findMany({
            where: {
        ...
        applicants.forEach((a, idx) => {
            ...
            doc.text(`Address: ${[a.contact?.address, a.contact?.city, a.contact?.state, a.contact?.zip].filter(Boolean).join(", ")}`);
            doc.moveDown(0.5);
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "printW2Batch failed", 500);
    }
}
// ── 1099 FORMS ───────────────────────────────────────────────────────────
/**
 ...
 */
async function list1099Contractors(req, res) {
    try {
        const taxYear = Number(req.query.taxYear ?? new Date().getFullYear());
        const yearStart = new Date(`${taxYear}-01-01T00:00:00.000Z`);
        const yearEnd = new Date(`${taxYear}-12-31T23:59:59.999Z`);
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "list1099Contractors failed", 500);
    }
}
/** GET /api/reports/1099/:applicantId/:taxYear/pdf */
async function generate1099PDF(req, res) {
    try {
        const { applicantId } = req.params;
        const taxYear = Number(req.params.taxYear ?? new Date().getFullYear());
        const yearStart = new Date(`${taxYear}-01-01T00:00:00.000Z`);
        ...
        if (!applicant)
            return (0, response_1.sendError)(res, "Applicant not found", 404);
        const gross = await prisma_config_1.default.payrollCheck.aggregate({
            _sum: { gross_pay: true },
            where: { applicant_id: applicantId, batch: { status: "POSTED", check_date: { gte: yearStart, lte: yearEnd } } },
        });
        const doc = new pdfkit_1.default({ margin: 40, size: "A4" });
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "generate1099PDF failed", 500);
    }
}
 ...
 */
async function notifyTaxDocumentAvailable(req, res) {
    try {
        const { applicantId } = req.params;
        const docType = req.body.docType === "1099" ? "1099" : "W2";
        ...
        if (!applicant?.contact?.email)
            return (0, response_1.sendError)(res, "Applicant email not on file", 400);
        ...
        await prisma_config_1.default.applicantCommunication.create({
            data: {
        ...
        return (0, response_1.sendSuccess)(res, { emailSent: result.success, error: result.error ?? null });
    }
    catch (err) {
        console.error(err);
        return (0, response_1.sendError)(res, "notifyTaxDocumentAvailable failed", 500);
    }
}
/**
 ...
 */
async function recordTaxDocumentConsent(req, res) {
    try {
        const { applicantId } = req.params;
        const docType = req.body.docType === "1099" ? "1099" : "W2";
        ...
        if (!applicant)
            return (0, response_1.sendError)(res, "Applicant not found", 404);
        const log = await prisma_config_1.default.applicantCommunication.create({
            data: {
                applicant_id: applicantId,
                ...
                sent_by_user_id: req.user?.user_id ?? null, metadata: { docType, consentGiven, source: "tax-document-consent" },
            },
        });
        return (0, response_1.sendSuccess)(res, { message: "Consent recorded", log });
    }
    catch (err) {
        console.error(err);
        return (0, response_1.sendError)(res, "recordTaxDocumentConsent failed", 500);
    }
}
// ═════════════════════════════════════════════════════════════════════════
...
async function payrollRegister(req, res) {
    try {
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "payrollRegister failed", 500);
    }
}
...
async function payrollSummary(req, res) {
    try {
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "payrollSummary failed", 500);
    }
}
...
async function earningsReport(req, res) {
    try {
        const dateRange = parseDateFilter(req);
        const lines = await prisma_config_1.default.payrollCheckLine.findMany({
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "earningsReport failed", 500);
    }
}
/** GET /api/reports/payroll/deductions?startDate=&endDate=&branch= */
async function deductionReport(req, res) {
    try {
        const dateRange = parseDateFilter(req);
        const branchFilter = req.query.branch;
        const [deductions, garnishments] = await Promise.all([
            prisma_config_1.default.benefitDeduction.findMany({
            ...
            prisma_config_1.default.garnishment.findMany({
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "deductionReport failed", 500);
    }
}
/** GET /api/reports/payroll/tax-liability?startDate=&endDate= */
async function taxLiabilityReport(req, res) {
    try {
        const dateRange = parseDateFilter(req);
        const checks = await prisma_config_1.default.payrollCheck.findMany({
            where: { batch: dateRange ? { check_date: dateRange } : undefined },
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "taxLiabilityReport failed", 500);
    }
}
/** GET /api/reports/payroll/ach-register?startDate=&endDate= */
async function achRegister(req, res) {
    try {
        const dateRange = parseDateFilter(req);
        const files = await prisma_config_1.default.aCHFile.findMany({
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "achRegister failed", 500);
    }
}
// ── BILLING REPORTS ──────────────────────────────────────────────────────
/** GET /api/reports/billing/invoice-register?startDate=&endDate=&statuses=&orgId= */
async function invoiceRegister(req, res) {
    try {
        const dateRange = parseDateFilter(req);
        const statusFilter = parseCSVParam(req.query.statuses);
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "invoiceRegister failed", 500);
    }
}
/** GET /api/reports/billing/revenue-by-customer?startDate=&endDate= */
async function revenueByCustomer(req, res) {
    try {
        const dateRange = parseDateFilter(req);
        const invoices = await prisma_config_1.default.clientInvoice.findMany({
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "revenueByCustomer failed", 500);
    }
}
/** GET /api/reports/billing/revenue-by-employee?startDate=&endDate= */
async function revenueByEmployee(req, res) {
    try {
        const dateRange = parseDateFilter(req);
        const lines = await prisma_config_1.default.clientInvoiceLine.findMany({
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "revenueByEmployee failed", 500);
    }
}
/** GET /api/reports/billing/revenue-by-branch?startDate=&endDate= */
async function revenueByBranch(req, res) {
    try {
        const dateRange = parseDateFilter(req);
        const invoices = await prisma_config_1.default.clientInvoice.findMany({
            where: dateRange ? { invoice_date: dateRange } : undefined,
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "revenueByBranch failed", 500);
    }
}
...
async function outstandingInvoices(req, res) {
    try {
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "outstandingInvoices failed", 500);
    }
}
/** GET /api/reports/billing/customer-summary */
async function customerBillingSummary(req, res) {
    try {
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "customerBillingSummary failed", 500);
    }
}
// ── COMPLIANCE REPORTS ───────────────────────────────────────────────────
/**
 ...
 */
async function acaEligibilityReport(req, res) {
    try {
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "acaEligibilityReport failed", 500);
    }
}
/** GET /api/reports/compliance/workers-comp?startDate=&endDate= */
async function workersCompensationReport(req, res) {
    try {
        const dateRange = parseDateFilter(req);
        const checks = await prisma_config_1.default.payrollCheck.findMany({
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "workersCompensationReport failed", 500);
    }
}
/** GET /api/reports/compliance/paid-sick-leave?startDate=&endDate= */
async function paidSickLeaveReport(req, res) {
    try {
        const dateRange = parseDateFilter(req);
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "paidSickLeaveReport failed", 500);
    }
}
/** GET /api/reports/compliance/employee-hours?startDate=&endDate=&branch= */
async function employeeHoursReport(req, res) {
    try {
        const dateRange = parseDateFilter(req);
        const branchFilter = req.query.branch;
        const entries = await prisma_config_1.default.timeEntry.findMany({
        ...
        for (const e of entries) {
            ...
            const acc = byEmployee.get(key) ?? { branch, customer: e.assignment.application.job.organization.name, regular: 0, ot: 0, total: 0 };
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "employeeHoursReport failed", 500);
    }
}
/** Shared directory used by w2EmployeesReport / contractors1099Report / activeAssignmentReport below. */
async function buildAssignmentDirectory(employmentType, branchFilter, activeOnly = false) {
    const assignments = await prisma_config_1.default.assignment.findMany({
        where: {
...
async function w2EmployeesReport(req, res) {
    try {
        const rows = await buildAssignmentDirectory("W2", req.query.branch);
        return respondWithReport(req, res, {
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "w2EmployeesReport failed", 500);
    }
}
/** GET /api/reports/compliance/1099-contractors?branch= */
async function contractors1099Report(req, res) {
    try {
        const rows = await buildAssignmentDirectory("1099", req.query.branch);
        return respondWithReport(req, res, {
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "contractors1099Report failed", 500);
    }
}
// ═════════════════════════════════════════════════════════════════════════
 ...
 */
async function newHireReport(req, res) {
    try {
        const dateRange = parseDateFilter(req);
        const branch = req.query.branch;
        const orgId = req.query.orgId;
        const jobTitle = req.query.jobTitle;
        const employeeType = req.query.employeeType; // "W2" | "1099"
        const assignments = await prisma_config_1.default.assignment.findMany({
            where: {
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "newHireReport failed", 500);
    }
}
/**
 ...
 */
async function oshaReport(req, res) {
    try {
        const rows = [];
        const format = getFormat(req);
        if (format === "json") {
            return (0, response_1.sendSuccess)(res, {
                count: 0,
                rows,
                message: "OSHA incident tracking is not yet modeled in the schema (no claim/incident table). This endpoint is wired and ready — add an OSHAIncident model to populate it.",
            });
        }
        return respondWithReport(req, res, {
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "oshaReport failed", 500);
    }
}
/**
 ...
 */
async function federalEEOReport(req, res) {
    try {
        const branch = req.query.branch;
        const dateRange = parseDateFilter(req);
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "federalEEOReport failed", 500);
    }
}
/**
 ...
 */
async function accruedHoursReport(req, res) {
    try {
        const dateRange = parseDateFilter(req);
        const orgId = req.query.orgId;
        const branch = req.query.branch;
        const assignedFilter = req.query.assigned; // "Y" | "N"
        const assignments = await prisma_config_1.default.assignment.findMany({
            where: {
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "accruedHoursReport failed", 500);
    }
}
/**
 ...
 */
async function activeAssignmentReport(req, res) {
    try {
        const branch = req.query.branch;
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "activeAssignmentReport failed", 500);
    }
}
/**
 ...
 */
async function deductionSubmittalReport(req, res) {
    try {
        const dateRange = parseDateFilter(req);
        const category = req.query.deductionCategory;
        const groupBy = req.query.groupBy || "branch";
        const deductions = await prisma_config_1.default.benefitDeduction.findMany({
            where: {
                is_active: true,
        ...
        const key = (d) => (groupBy === "deduction" ? d.deduction_type : d.applicant.office_division ?? "Unassigned");
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "deductionSubmittalReport failed", 500);
    }
}
/**
 ...
 */
async function employeeDeductionContributionReport(req, res) {
    try {
        const branch = req.query.branch;
        const [deductions, garnishments, bankAccounts] = await Promise.all([
            ...
            prisma_config_1.default.bankAccount.findMany({
                where: { is_active: true },
                select: {
                    bank_name: true,
                    amount: true,
        ...
        console.error(err);
        return (0, response_1.sendError)(res, "employeeDeductionContributionReport failed", 500);
    }
}
// ═════════════════════════════════════════════════════════════════════════
...
async function getReportCatalog(req, res) {
    const catalog = {
        yearEndTaxDocuments: [
    ...
    return (0, response_1.sendSuccess)(res, catalog);
}
// ═════════════════════════════════════════════════════════════════════════
