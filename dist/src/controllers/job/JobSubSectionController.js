...
exports.getJobInterviews = exports.getJobOverview = exports.getTimesheetEntries = exports.getJobTimesheetCounts = exports.getJobTimesheets = exports.getJobAssignmentCounts = exports.getJobAssignments = exports.getJobPipelineCounts = exports.getJobPipelinedApplicants = exports.getJobApplicationsCounts = exports.getJobApplications = void 0;
 ...
 */
async function requireJob(jobId, res) {
...
async function jobExists(jobId) {
    const count = await prisma_config_1.default.job.count({ where: { job_id: jobId } });
    return count > 0;
}
// ============================================================
...
const getJobApplications = async (req, res) => {
    try {
        ...
        const { status, source, search, applied_from, applied_to, sort_by = 'applied_at', sort_dir = 'desc', } = req.query;
        // ── WHERE clause ────────────────────────────────────────
        const where = { job_id };
        if (status)
        ...
        if (applied_from || applied_to) {
            where.applied_at = {};
            if (applied_from)
                where.applied_at.gte = new Date(applied_from);
            if (applied_to)
                where.applied_at.lte = new Date(applied_to);
        }
        if (search) {
            where.applicant = {
                OR: [
        ...
        const [applications, total] = await Promise.all([
            ...
            prisma_config_1.default.application.count({ where }),
        ]);
        if (total === 0) {
        ...
        return (0, response_1.sendError)(res, 'Failed to fetch job applications', 500);
    }
};
 ...
 */
const getJobApplicationsCounts = async (req, res) => {
    try {
        const { id: job_id } = req.params;
        const [total, byStatus, withInterview, hired] = await Promise.all([
            prisma_config_1.default.application.count({ where: { job_id } }),
            ...
            prisma_config_1.default.application.count({ where: { job_id, status: 'HIRED' } }),
        ]);
        const statusMap = Object.fromEntries(byStatus.map(s => [s.status, s._count.application_id]));
        if (total === 0) {
            if (!(await jobExists(job_id)))
                return (0, response_1.sendError)(res, 'Job not found', 404);
        }
        ...
        return (0, response_1.sendError)(res, 'Failed to fetch application counts', 500);
    }
};
exports.getJobApplicationsCounts = getJobApplicationsCounts;
// ============================================================
 ...
 *   sort_dir       asc | desc         default: desc
 */
const getJobPipelinedApplicants = async (req, res) => {
    try {
        const { id: job_id } = req.params;
        const { page, limit, skip } = parsePagination(req.query);
        const { stage, search, from_date, to_date, sort_by = 'pipeline_date', sort_dir = 'desc', } = req.query;
        // ── WHERE on PipelineStage ───────────────────────────────
        const stageWhere = {};
        if (stage)
            stageWhere.stage_name = stage.toUpperCase();
        if (from_date || to_date) {
            stageWhere.pipeline_date = {};
            if (from_date)
                stageWhere.pipeline_date.gte = new Date(from_date);
            if (to_date)
                stageWhere.pipeline_date.lte = new Date(to_date);
        }
        // ── WHERE on Application (→ Job) ────────────────────────
        const applicationWhere = { job_id };
        if (search) {
        ...
        return (0, response_1.sendError)(res, 'Failed to fetch pipelined applicants', 500);
    }
};
exports.getJobPipelinedApplicants = getJobPipelinedApplicants;
// ── Pipeline stage counts ────────────────────────────────────
const getJobPipelineCounts = async (req, res) => {
    try {
        const { id: job_id } = req.params;
        const grouped = await prisma_config_1.default.pipelineStage.groupBy({
            by: ['stage_name'],
            where: { application: { job_id } },
            _count: { pipeline_stage_id: true },
        });
        const map = Object.fromEntries(grouped.map(g => [g.stage_name, g._count.pipeline_stage_id]));
        ...
        return (0, response_1.sendError)(res, 'Failed to fetch pipeline counts', 500);
    }
};
exports.getJobPipelineCounts = getJobPipelineCounts;
// ============================================================
...
const getJobAssignments = async (req, res) => {
    try {
        const { id: job_id } = req.params;
        const { page, limit, skip } = parsePagination(req.query);
        const { employment_type, search, start_from, start_to, end_from, end_to, active_only, falloff, sort_by = 'start_date', sort_dir = 'desc', } = req.query;
        // ── WHERE on Assignment ──────────────────────────────────
        const assignmentWhere = {};
        if (employment_type)
            assignmentWhere.employment_type = employment_type.toUpperCase();
        if (falloff !== undefined)
            assignmentWhere.falloff = falloff === 'true';
        if (start_from || start_to) {
            assignmentWhere.start_date = {};
            if (start_from)
                assignmentWhere.start_date.gte = new Date(start_from);
            if (start_to)
                assignmentWhere.start_date.lte = new Date(start_to);
        }
        if (end_from || end_to) {
            assignmentWhere.end_date = {};
            if (end_from)
                assignmentWhere.end_date.gte = new Date(end_from);
            if (end_to)
                assignmentWhere.end_date.lte = new Date(end_to);
        }
        if (active_only === 'true') {
            const now = new Date();
        ...
        return (0, response_1.sendError)(res, 'Failed to fetch job assignments', 500);
    }
};
exports.getJobAssignments = getJobAssignments;
// ── Assignment counts ────────────────────────────────────────
const getJobAssignmentCounts = async (req, res) => {
    try {
        const { id: job_id } = req.params;
        const now = new Date();
        const [total, active, w2, contractor, falloffs, extended] = await Promise.all([
            prisma_config_1.default.assignment.count({ where: { application: { job_id } } }),
            ...
            prisma_config_1.default.assignment.count({ where: { application: { job_id }, employment_type: 'W2' } }),
            prisma_config_1.default.assignment.count({
                where: { application: { job_id }, employment_type: 'CONTRACTOR_1099' },
            }),
            prisma_config_1.default.assignment.count({ where: { application: { job_id }, falloff: true } }),
            prisma_config_1.default.assignment.count({ where: { application: { job_id }, extended: true } }),
        ]);
        if (total === 0) {
            if (!(await jobExists(job_id)))
                return (0, response_1.sendError)(res, 'Job not found', 404);
        }
        return (0, response_1.sendSuccess)(res, { total, active, w2, contractor, falloffs, extended });
    }
    catch (err) {
        console.error('getJobAssignmentCounts error:', err);
        return (0, response_1.sendError)(res, 'Failed to fetch assignment counts', 500);
    }
};
exports.getJobAssignmentCounts = getJobAssignmentCounts;
// ============================================================
 ...
 *   sort_dir       asc | desc        default: desc
 */
const getJobTimesheets = async (req, res) => {
    try {
        const { id: job_id } = req.params;
        const { page, limit, skip } = parsePagination(req.query);
        const { status, search, week_from, week_to, assignment_id, qb_synced, sort_by = 'week_start_date', sort_dir = 'desc', } = req.query;
        // ── WHERE on Timesheet ───────────────────────────────────
        const where = {
            assignment: {
                application: {
        ...
        if (qb_synced !== undefined)
            where.qb_synced = qb_synced === 'true';
        if (week_from || week_to) {
            where.week_start_date = {};
            if (week_from)
                where.week_start_date.gte = new Date(week_from);
            if (week_to)
                where.week_start_date.lte = new Date(week_to);
        }
        // ── ORDER BY ─────────────────────────────────────────────
        const validSortFields = {
        ...
        return (0, response_1.sendError)(res, 'Failed to fetch job timesheets', 500);
    }
};
exports.getJobTimesheets = getJobTimesheets;
// ── Timesheet aggregate counts ───────────────────────────────
const getJobTimesheetCounts = async (req, res) => {
    try {
        const { id: job_id } = req.params;
        const baseWhere = { assignment: { application: { job_id } } };
        const [total, byStatus, aggHours, aggBill] = await Promise.all([
            prisma_config_1.default.timesheet.count({ where: baseWhere }),
        ...
        const statusMap = Object.fromEntries(byStatus.map(s => [s.status, s._count.timesheet_id]));
        if (total === 0) {
            if (!(await jobExists(job_id)))
                return (0, response_1.sendError)(res, 'Job not found', 404);
        }
        ...
        return (0, response_1.sendError)(res, 'Failed to fetch timesheet counts', 500);
    }
};
exports.getJobTimesheetCounts = getJobTimesheetCounts;
// ============================================================
...
const getTimesheetEntries = async (req, res) => {
    try {
        ...
        const timesheet = await prisma_config_1.default.timesheet.findFirst({
            where: {
        ...
        if (!timesheet) {
            return (0, response_1.sendError)(res, 'Timesheet not found for this job', 404);
        }
        const { work_type, from_date, to_date } = req.query;
        const { page, limit, skip } = parsePagination(req.query, 31, 31); // max 31 days
        const where = { timesheet_id: timesheetId };
        if (work_type)
            where.work_type = work_type.toUpperCase();
        if (from_date || to_date) {
            where.work_date = {};
            if (from_date)
                where.work_date.gte = new Date(from_date);
            if (to_date)
                where.work_date.lte = new Date(to_date);
        }
        ...
        return (0, response_1.sendError)(res, 'Failed to fetch timesheet entries', 500);
    }
};
exports.getTimesheetEntries = getTimesheetEntries;
// ============================================================
 ...
 */
const getJobOverview = async (req, res) => {
    try {
        const { id: job_id } = req.params;
        ...
        if (!job)
            return;
        const now = new Date();
        const [appTotal, appByStatus, pipelineGrouped, assignmentTotal, activeAssignments, timesheetTotal, timesheetByStatus, hoursAggregate, interviewTotal,] = await Promise.all([
            // Applications
            prisma_config_1.default.application.count({ where: { job_id } }),
            prisma_config_1.default.application.groupBy({
                by: ['status'],
            ...
            // Timesheets
            prisma_config_1.default.timesheet.count({ where: { assignment: { application: { job_id } } } }),
            prisma_config_1.default.timesheet.groupBy({
                by: ['status'],
            ...
            // Interviews
            prisma_config_1.default.interview.count({ where: { application: { job_id } } }),
        ]);
        ...
        return (0, response_1.sendError)(res, 'Failed to fetch job overview', 500);
    }
};
exports.getJobOverview = getJobOverview;
// ============================================================
...
const getJobInterviews = async (req, res) => {
    try {
        const { id: job_id } = req.params;
        const { page, limit, skip } = parsePagination(req.query);
        const { status, interview_type, round, search, from_date, to_date, sort_by = 'interview_date', sort_dir = 'desc', } = req.query;
        const where = { application: { job_id } };
        if (status)
            where.status = status.toUpperCase();
        if (interview_type)
            where.interview_type = interview_type.toUpperCase();
        if (round)
            where.round = parseInt(round);
        if (from_date || to_date) {
            where.interview_date = {};
            if (from_date)
                where.interview_date.gte = new Date(from_date);
            if (to_date)
                where.interview_date.lte = new Date(to_date);
        }
        if (search) {
            where.application = {
                job_id,
        ...
        const [interviews, total] = await Promise.all([
            ...
            prisma_config_1.default.interview.count({ where }),
        ]);
        if (total === 0) {
            if (!(await jobExists(job_id)))
                return (0, response_1.sendError)(res, 'Job not found', 404);
        ...
        return (0, response_1.sendError)(res, 'Failed to fetch job interviews', 500);
    }
};
exports.getJobInterviews = getJobInterviews;
//# sourceMappingURL=JobSubSectionController.js.map