...
exports.getLiveStatus = exports.getAttendanceSummary = exports.updateAttendanceRecord = exports.getAttendanceByDate = exports.getAllAttendance = exports.getUserAttendance = exports.getTodayStatus = exports.resumeClock = exports.pauseClock = exports.clockOut = exports.clockIn = void 0;
...
const recalculateAttendanceTotals = async (attendance_id) => {
    const sessions = await prisma_config_1.default.clockSession.findMany({
        where: { attendance_id },
    ...
    const lastClockOut = closedSessions.length
        ...
        : null;
    return prisma_config_1.default.attendanceLog.update({
        where: { attendance_id },
        data: {
            total_hours: new client_1.Prisma.Decimal((totalMinutes / 60).toFixed(2)),
...
const clockIn = async (req, res) => {
    try {
        ...
        let attendance = await prisma_config_1.default.attendanceLog.findUnique({
            where: { user_id_work_date: { user_id, work_date } },
            include: { sessions: true },
        });
        if (!attendance) {
            attendance = await prisma_config_1.default.attendanceLog.create({
                data: { user_id, work_date, status: client_1.AttendanceStatus.CLOCKED_OUT },
                include: { sessions: true },
            });
        }
        const alreadyOpen = attendance.sessions.some(s => !s.clock_out_at);
        if (attendance.status === client_1.AttendanceStatus.CLOCKED_IN || alreadyOpen) {
            return res.status(400).json({ error: 'User is already clocked in' });
        }
        if (attendance.status === client_1.AttendanceStatus.ON_BREAK) {
            return res.status(400).json({ error: 'User is on a break — use /attendance/resume instead' });
        }
        await prisma_config_1.default.clockSession.create({
            data: {
                attendance_id: attendance.attendance_id,
                clock_in_at: now,
                clock_in_lat: latitude !== undefined ? new client_1.Prisma.Decimal(latitude) : undefined,
                clock_in_lng: longitude !== undefined ? new client_1.Prisma.Decimal(longitude) : undefined,
                clock_in_ip: ip,
            },
        });
        await prisma_config_1.default.attendanceLog.update({
            where: { attendance_id: attendance.attendance_id },
            data: { status: client_1.AttendanceStatus.CLOCKED_IN },
        });
        const updated = await recalculateAttendanceTotals(attendance.attendance_id);
        const full = await prisma_config_1.default.attendanceLog.findUnique({
...
const clockOut = async (req, res) => {
    try {
        ...
        const now = new Date();
        const ip = req.headers['x-forwarded-for'] || req.socket.remoteAddress || undefined;
        const attendance = await prisma_config_1.default.attendanceLog.findUnique({
            where: { user_id_work_date: { user_id, work_date } },
            include: { sessions: true },
        });
        if (!attendance) {
        ...
        if (attendance.status === client_1.AttendanceStatus.CLOCKED_OUT) {
        ...
        if (openSession) {
            await prisma_config_1.default.clockSession.update({
                where: { session_id: openSession.session_id },
                data: {
                    clock_out_at: now,
                    clock_out_lat: latitude !== undefined ? new client_1.Prisma.Decimal(latitude) : undefined,
                    clock_out_lng: longitude !== undefined ? new client_1.Prisma.Decimal(longitude) : undefined,
                    clock_out_ip: ip,
        ...
        await prisma_config_1.default.attendanceLog.update({
            where: { attendance_id: attendance.attendance_id },
            data: { status: client_1.AttendanceStatus.CLOCKED_OUT },
        });
        const updated = await recalculateAttendanceTotals(attendance.attendance_id);
        const full = await prisma_config_1.default.attendanceLog.findUnique({
            where: { attendance_id: updated.attendance_id },
...
// ─────────────────────────────────────────────────────────────
const pauseClock = async (req, res) => {
    try {
        ...
        if (!attendance || attendance.status !== client_1.AttendanceStatus.CLOCKED_IN) {
        ...
        await prisma_config_1.default.attendanceLog.update({
            where: { attendance_id: attendance.attendance_id },
            data: { status: client_1.AttendanceStatus.ON_BREAK },
        });
        const updated = await recalculateAttendanceTotals(attendance.attendance_id);
        const full = await prisma_config_1.default.attendanceLog.findUnique({
            where: { attendance_id: updated.attendance_id },
...
// ─────────────────────────────────────────────────────────────
const resumeClock = async (req, res) => {
    try {
        const { user_id, latitude, longitude } = req.body;
        if (!user_id) {
            return res.status(400).json({ error: 'user_id is required' });
        ...
        if (!attendance || attendance.status !== client_1.AttendanceStatus.ON_BREAK) {
    ...
    try {
        ...
        if (req.query.start_date || req.query.end_date) {
            ...
            if (req.query.start_date)
                where.work_date.gte = toDateOnlyUTC(req.query.start_date);
            if (req.query.end_date)
                where.work_date.lte = toDateOnlyUTC(req.query.end_date);
        }
        if (req.query.status) {
            where.status = req.query.status.toUpperCase();
        }
        const [logs, total] = await Promise.all([
            prisma_config_1.default.attendanceLog.findMany({
                where,
                skip,
                take: limit,
            ...
            prisma_config_1.default.attendanceLog.count({ where }),
        ]);
        res.json({
            data: logs,
            paging: {
...
const getAllAttendance = async (req, res) => {
    try {
        ...
        const where = {};
        if (status && Object.values(client_1.AttendanceStatus).includes(status.toUpperCase())) {
...
const updateAttendanceRecord = async (req, res) => {
    try {
        ...
        const existing = await prisma_config_1.default.attendanceLog.findUnique({ where: { attendance_id: attendanceId } });
        if (!existing) {
            return res.status(404).json({ error: 'Attendance record not found' });
        }
        if (status && !Object.values(client_1.AttendanceStatus).includes(status)) {
            return res.status(400).json({ error: 'Invalid status value' });
        }
        const updateData = { is_manual_entry: true };
        if (first_clock_in_at !== undefined)
            updateData.first_clock_in_at = new Date(first_clock_in_at);
        if (last_clock_out_at !== undefined)
            updateData.last_clock_out_at = new Date(last_clock_out_at);
        if (total_hours !== undefined)
            updateData.total_hours = new client_1.Prisma.Decimal(total_hours);
        if (status !== undefined)
            updateData.status = status;
        if (notes !== undefined)
            updateData.notes = notes;
        const updated = await prisma_config_1.default.attendanceLog.update({
...
const getAttendanceSummary = async (req, res) => {
    try {
        ...
        const logs = await prisma_config_1.default.attendanceLog.findMany({
            where,
...
const getLiveStatus = async (req, res) => {
    try {
        ...
        else if (statusFilter && Object.values(client_1.AttendanceStatus).includes(statusFilter)) {
            where.attendance_logs = { some: { work_date, status: statusFilter } };
        }
        const [users, total, clockedIn, onBreak, clockedOut, totalActive] = await Promise.all([
            ...
            prisma_config_1.default.user.count({ where: { ...baseWhere, attendance_logs: { some: { work_date, status: client_1.AttendanceStatus.CLOCKED_IN } } } }),
            prisma_config_1.default.user.count({ where: { ...baseWhere, attendance_logs: { some: { work_date, status: client_1.AttendanceStatus.ON_BREAK } } } }),
            prisma_config_1.default.user.count({ where: { ...baseWhere, attendance_logs: { some: { work_date, status: client_1.AttendanceStatus.CLOCKED_OUT } } } }),
            prisma_config_1.default.user.count({ where: baseWhere }),
        ]);
