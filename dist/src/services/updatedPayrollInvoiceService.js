...
exports.generateClientInvoicePdf = void 0;
...
const fetchClientInvoiceData = async (invoiceId) => {
    ...
    if (!invoice)
        throw new Error(`Client invoice ${invoiceId} not found`);
...
// ─── PDF Builder ─────────────────────────────────────────────────
const buildClientInvoicePdf = (data) => {
    return new Promise((resolve, reject) => {
        ...
        const money = (v) => '$' + parseFloat(String(v)).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
        ...
        if (data.bill_to_address) {
            doc.font('Helvetica').fontSize(8.5).fillColor(C.TEXT).text(data.bill_to_address, L + pad, c1y, { width: colW - pad * 2, lineBreak: false });
            c1y += lineH;
        }
        if (data.bill_to_email)
            doc.font('Helvetica').fontSize(8.5).fillColor(C.TEXT).text(data.bill_to_email, L + pad, c1y, { width: colW - pad * 2, lineBreak: false });
        let c2y = y + pad;
...
// ─── Main Export ─────────────────────────────────────────────────
const generateClientInvoicePdf = async (invoiceId) => {
...
exports.generateClientInvoicePdf = generateClientInvoicePdf;
//# sourceMappingURL=updatedPayrollInvoiceService.js.map