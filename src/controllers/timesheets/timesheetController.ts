...
const computeBilling = async (
  ...
  if (!rate) {
    throw new Error(
      `No JobRate configured for job "${assignment.application.job.job_title}". ` +
      `Please add a billing rate before approving.`
    );
  }

  const billRate   = new Decimal(rate.bill_rate);
  const otBillRate = rate.ot_bill_rate ? new Decimal(rate.ot_bill_rate) : billRate.mul(1.5);
  const payRate    = rate.pay_rate     ? new Decimal(rate.pay_rate)     : new Decimal(0);
  const otPayRate  = rate.ot_pay_rate  ? new Decimal(rate.ot_pay_rate)  : payRate.mul(1.5);
